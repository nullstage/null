...
### 조선굵은고딕 (ChosunKg) — `src/assets/fonts/ChosunKg.ttf`

대화 본문과 UI 모노스페이스 자리에 쓴다.
폰트 파일의 `name` 테이블 기준 정보다.
...
조선일보가 배포하는 무료 서체다. 공개된 이용 조건을 요약하면 다음과 같다.

- 개인·기업 무료 사용. 인쇄물·웹페이지·영상 등 용도 제한이 넓다.
- 수정 후 판매 금지. "배포된 형태 그대로" 사용해야 한다.
- 복사·배포에 대가를 요구할 수 없다.
- **공식 배포처 이외에서의 배포를 금지한다.**

**미해결 사항.** 이 프로젝트는 `next/font/local`로 ttf를 번들해
GitHub Pages에서 직접 서빙한다. 마지막 조항("공식 배포처 이외 배포 금지")에
웹폰트 서빙이 해당하는지가 확정되지 않았다. 조선일보 공식 배포 페이지의
원문 확인이 필요하고, 조건에 맞지 않으면 라이선스가 명확한 서체
(예: OFL 계열)로 교체하는 것이 안전하다.

### Pretendard

시작 화면 메뉴와 Phaser 씬 텍스트에서 `'Pretendard', sans-serif`로 이름만 참조한다.
**서체 파일을 저장소에 두지 않으므로 재배포 대상이 아니다.**
사용자 시스템에 설치되어 있으면 적용되고 없으면 `sans-serif`로 대체된다.
